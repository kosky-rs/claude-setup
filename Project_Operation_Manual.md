...
5. [Issue管理と継続的改善](#5-issue管理と継続的改善)
...
│   │   ├── log-issue.md      # Issue記録コマンド
│   │   ├── retrospective.md  # セッション振り返りコマンド
...
```
You: /log-issue テストを実行せずにコミットしてしまった
...
**Issue分類:**
- `quality-gate` - テスト・検証の問題
- `context-management` - コンテキスト管理の問題
- `workflow` - プロセス・手順の問題
- `code-standard` - コーディング規約違反
- `communication` - 確認・報告の問題
...
プロジェクト終了時またはIssueが蓄積したら：

```
You: /sync-issues-to-global
...
### 6.1 グローバルコマンド（どのプロジェクトでも使用可）

| コマンド | 説明 | 使用タイミング |
|----------|------|----------------|
| `/checkpoint` | 現在の状態を保存 | コンテキスト60%超、複雑な作業前 |
| `/review` | コードレビュー実行 | コミット前 |
| `/upgrade-global` | グローバル設定をアップグレード | 週次メンテナンス時 |
| `/verify-upgrade` | アップグレードの効果検証 | アップグレード後5セッション経過時 |
...
| `/log-issue [説明]` | Issue記録 | 問題発生時 |
| `/retrospective` | セッション振り返り | セッション終了時 |
| `/sync-issues-to-global` | Issueをグローバルに同期 | プロジェクト終了時、Issue蓄積時 |
...
| 1.1.0 | 2025-12-02 | Issue管理と継続的改善システムを追加 |
